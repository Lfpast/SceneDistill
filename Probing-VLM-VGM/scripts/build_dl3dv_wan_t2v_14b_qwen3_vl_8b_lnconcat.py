"""Build offline DL3DV features by branch-LN concat of WAN-14B and Qwen3VL.

Default input layout:
  <wan-feat-root>/wan-t2v-14b/<subset>/<hash>/feature_t749_layer20.sft
  <qwen-feat-root>/qwen3-vl-8b/<subset>/<hash>/feature_layer22.sft

Default output layout:
  <out-feat-root>/wan-t2v-14b-qwen3-vl-8b-lnconcat/<subset>/<hash>/feature_layer22.sft
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Root containing DL3DV train.json / val.json split files.",
    )
    parser.add_argument(
        "--wan-feat-root",
    ...
    parser.add_argument("--wan-postfix", default="_t749_layer20")
...
def _iter_split_scenes(processed_root: Path, split: str) -> Iterable[tuple[str, str]]:
    ...
    with split_file.open("r") as f:
        pairs = json.load(f)
    for subset, scene_hash in pairs:
        yield str(subset), str(scene_hash)
...
def _build_one(
    ...
    out_dir = out_feat_root / args.out_vfm_name / subset / scene_hash
    ...
    if fused.shape != (*qwen_feat.shape[:3], expected_c):
        raise RuntimeError(
            f"Bad fused shape for {subset}/{scene_hash}: {tuple(fused.shape)}"
        )
...
def main() -> None:
    ...
    wan_feat_root = Path(args.wan_feat_root)
    qwen_feat_root = Path(args.qwen_feat_root)
    out_feat_root = Path(args.out_feat_root)
    ...
    for split in args.splits:
        ...
        for i, (subset, scene_hash) in enumerate(scenes, start=1):
            status = _build_one(
                wan_feat_root,
                qwen_feat_root,
                out_feat_root,
            ...
            if i == 1 or i % 25 == 0 or i == total:
                print(f"[{split}] {i}/{total} {subset}/{scene_hash}: {status}")
