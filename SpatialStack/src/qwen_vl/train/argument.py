...
class ModelArguments:
    ...
    geometry_encoder_type: str = field(default="vggt")  # "vggt", "vggt_omega", "vggt_omega_direct", or "scene_distill"
    geometry_encoder_path: str = field(default="facebook/VGGT-1B/")  # Path or repo id for the pre-trained geometry encoder
    geometry_encoder_freeze: bool = field(default=True)  # Keep the geometry encoder frozen
...
class DataArguments:
    ...
    video_min_frames: Optional[int] = field(default=8)
    base_interval: int = field(default=1)
    video_max_frame_pixels: int = field(default=512 * 512)
    video_min_frame_pixels: int = field(default=28 * 28 * 16)
